...
SKILL.md 是路由器，通过 Quick Reference 表格用最少 token 完成路由判断。引用文件的“契约式”写法，即契约式引用会告诉 Claude 何时加载、加载什么、得到什么。这里有一个 500 行法则，超过就该重构——把参考、模板、示例移到辅助文件。通过脚本封装来确定性逻辑用脚本，Claude 执行但不需要理解脚本中的内容。
