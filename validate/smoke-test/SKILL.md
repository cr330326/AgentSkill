...
3. 评估执行风险。任何会请求真实服务的动作都必须先经过用户明确确认。执行前加载 `references/execution_policy.md`。
4. 解析 OpenAPI/Swagger，并从 SPEC 中提取业务关键词，用于帮助筛选更有代表性的端点。端点筛选规则见 `references/openapi_scope.md`。
5. 使用 `scripts/generate_smoke_tests.py` 在隔离目录生成测试产物。不要写入项目根目录下的 `tests/`。
6. 如果用户确认执行，再使用 `scripts/run_smoke_tests.py` 运行 pytest，并把结果写入工作目录。
7. 按 `references/output_contract.md` 返回统一 JSON，并附带关键文件路径、跳过原因和下一步建议。
